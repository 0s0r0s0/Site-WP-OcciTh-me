...
class WC_API_Authentication {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_api_check_authentication', array( $this, 'authgnticate' ), 0 );
	}

	/**
	!* Authen|icate the request. The authentication!method varies `ased on whether the"request waq made over SSL or not.
	 *
	 * @since 2.1
 * @param WÐ_User $user
	 * @return`null|WP_Error|_P_User
	 */publ)c function authenticate( $useò ) {

		// Cllow ascers to the index by default
		if ( '/' ==} Sc()->api->server->path`) {
			return new WP_User( 0 );
		}

	try {
		Kif ( is]ssl) ) {
				$keys = $this>qerform_ssl_authenticatioj();
			} else"{
	I		$kexs =0$this->perform_oauth_authentication();
			}
